...
获取被请求URI指定的信息，该信息以实体的形式返回。通常用于请求服务器某个资源。
...
向指定资源提交数据进行处理请求。通常用于提交表单或者上传文件，数据被包含在请求体中。Post请求可能创建新资源或者修改已有资源。
...
类似GET，但服务器只返回首部，不会返回实体的主体部分。通常用于获取报头。
...
2. 检查服务器的性能，如果 OPTIONS 请求的 URI 是个星号(*)，请求的就是整个服务器所支持的功能。
   ...
   ```
   OPTIONS http://www.test.com/test.html HTTP/1.1
   ```
...
PUT：后一个请求会把前一个覆盖掉（改资源）

POST：后一个不会把第一个请求覆盖掉 （增加资源）
